...
export class SortingPipe implements PipeTransform {
  ...
  static caseInsensitiveSort(a: any, b: any) {
    if (SortingPipe.isString(a) && SortingPipe.isString(b)) {
      return a.localeCompare(b);
    }
    return SortingPipe.defaultCompare(a, b);
  }
  ...
  private sortArray(
    ...
    if (isDeepLink) {
      expression = SortingPipe.parseExpression(expression);
    }
    ...
    if (comparator && typeof comparator === "function") {
      ...
      compareFn = isCaseInsensitive
        ? SortingPipe.caseInsensitiveSort
        : SortingPipe.defaultCompare;
    }
    ...
    const array: any[] = value.sort(
      (a: any, b: any): number => {
        ...
        return compareFn(
          SortingPipe.getValue(a, expression),
          SortingPipe.getValue(b, expression)
        );
      }
    );
  ...
  private transformObject(
    ...
    const parsedExpression = SortingPipe.parseExpression(expression);
    let lastPredicate = parsedExpression.pop();
    let oldValue = SortingPipe.getValue(value, parsedExpression);

    if (!Array.isArray(oldValue)) {
      parsedExpression.push(lastPredicate);
      lastPredicate = null;
      oldValue = SortingPipe.getValue(value, parsedExpression);
    }
